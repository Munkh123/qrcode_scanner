...
def get_payments_by_customer_id(
    ...
    return db.query(Payment).filter(Payment.customer_id == customer_id).filter(Payment.is_checked==True).order_by(Payment.checkout_date.desc()).offset(offset).limit(limit).all()
